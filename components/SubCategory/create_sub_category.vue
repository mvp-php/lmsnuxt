    ...
    <span>
            ...
            <div class="modal-record-col2">
                <div class="slds-form-element__control  ">
                    <div class="slds-select_container cus-select-container">
                        <formDropdown :options="categoryList" v-model="subCategoryData.parent_category_id"
                            selectName="Major Category" class="slds-select custom-grid-input" @onChange="ChangeCategory"
                            :selected="`${subCategoryData.parent_category_id}`"></formDropdown>
                    </div>
                ...
                <div class="mb-0 user-modal-desc">
                    <div class="slds-form-element__control ">
                        <FormTextareaField rows="3" v-model="subCategoryData.description" no-resize
                            ...
                            :bindValue="`${subCategoryData.description}`" />
        ...
        <div class="modal-record-col2">
            <Dropzone v-bind:fileUploadSuccessEvent="fileUploadSuccessEvent" modelname="Dropzone"
                v-model="subCategoryData.dropzoneImage" :folder="`category`" :type="`image`"
                :extension="`.png, .jpeg, .jpg, .gif`" :existingImage="`${subCategoryData.image_name}`" />
        </div>
    </span>
</template>
<script>
